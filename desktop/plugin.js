...
const friendly = value => String(value || 'Unknown').replaceAll('_', ' ').replace(/^./, char => char.toUpperCase())
...
function ApprovalReview({ controller, state, run, onClose }) {
  ...
  return h(ControlDialog, { title: 'Review proposed actions', onClose }, h('div', { className: 'cr-stack' },
    ...
    ...(approval?.tasks || []).map((task, index) => h('section', { key: task.id || index, className: 'cr-panel' }, h('h3', null, task.title || task.description || friendly(task.functionName || task.tool || `Action ${index + 1}`)),
      ...
      h('dl', null, ...Object.entries(task).filter(([key]) => !['id', 'title', 'description', 'parameters', 'arguments'].includes(key)).concat(Object.entries(task.parameters || task.arguments || {})).flatMap(([key, value]) => [h('dt', { key: `${key}-label` }, friendly(key)), h('dd', { key }, readableActionValue(key, value, run, state.artifacts))])))),
...
function TaskDetail({ controller, state, open, onBack }) {
  ...
  return h('article', { className: 'cr-detail', 'aria-label': 'Selected workflow' },
    ...
    h('section', null, h('h3', null, 'Activity'), state.events.length ? h('ol', { className: 'cr-timeline' }, ...state.events.map((event, index) => h('li', { key: event.sequence ?? event.id ?? index }, h('div', null, h('p', null, event.presentation?.message || event.message || friendly(event.kind || event.type)), h('p', { className: 'cr-small cr-muted' }, `${dateLabel(event.occurredAt || event.at)}${event.presentation?.actorLabel || event.actorLabel ? ` · ${event.presentation?.actorLabel || event.actorLabel}` : ''}`))))) : h('p', { className: 'cr-muted' }, 'No activity entries are available yet.'),
