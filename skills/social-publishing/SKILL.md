...
**Enterprise identity:** Enterprise keys are ClipIt-team-only. Select the workspace's distinct non-default CLI profile with `CLIPIT_PROFILE`, then run `scripts/verify_enterprise_workspace.py --workspace-id <expected-id>` before listing accounts or publishing. Keep that profile selected for the whole client session. Never put a workspace key in the shared Hermes `CLIPPER_API_KEY` environment variable.
...
|-----------|----------------|---------------|
| List accounts | `clipit social accounts --json` | `scripts/list_social_accounts.py` |
| Check selected enterprise delivery | `clipit deliverables list --status selected --json` | `scripts/list_deliverables.py --status selected` |
| Post/schedule | `clipit social post|schedule ... --confirm --max-credits <cap> --json` | `scripts/post_to_social.py`; `scripts/schedule_social_post.py` |
| Status/cancel | `clipit social get|cancel ...` | `scripts/get_social_post.py`; `scripts/cancel_social_post.py` |
...
- The exact target account MUST appear in `scripts/list_social_accounts.py` immediately before publishing.
...
4. Prefer `clipit social post ... --confirm --max-credits <cap> --json`; use `scripts/post_to_social.py` only as the approved REST fallback.
...
**Steps:**
1. Refresh `scripts/list_social_accounts.py` and copy the exact authorized `accountId`.
2. Present the exact artifact/account/copy/time zone/time, live estimate, and cap; obtain schedule approval.
3. Prefer `clipit social schedule ... --confirm --max-credits <cap> --json`; use `scripts/schedule_social_post.py` only as the approved REST fallback.
...
- **Never identify an account by display name.** Refresh `scripts/list_social_accounts.py` and pin its exact `accountId`. Revoked enterprise grants disappear from this list.
...
- **Enterprise identity must be proven first.** A successful personal-library request is not workspace proof; require `scripts/verify_enterprise_workspace.py` for the expected workspace ID.
- **Agents cannot select client authority.** The recipe and `scripts/deliver_export.py` create `ready` only. Wait for the client to select in the portal and verify `selected` with the same named profile.
...
- [scripts/cancel_social_post.py](scripts/cancel_social_post.py)
...
- [scripts/get_social_post.py](scripts/get_social_post.py)
- [scripts/list_deliverables.py](scripts/list_deliverables.py)
- [scripts/list_social_accounts.py](scripts/list_social_accounts.py)
- [scripts/post_to_social.py](scripts/post_to_social.py)
- [scripts/schedule_social_post.py](scripts/schedule_social_post.py)
- [scripts/social_publish_contract.py](scripts/social_publish_contract.py)
