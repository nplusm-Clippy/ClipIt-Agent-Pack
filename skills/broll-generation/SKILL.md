...
|-----------|----------------|---------------|
| Plan concepts | `clipit broll plan <clipId> --count 3 --confirm --json` | `scripts/plan_broll.py --clip-id <id> --count 3` |
| Generate B-Roll | `clipit broll generate <clipId> --concept-index <n> --confirm --max-credits <cap> --json` | `scripts/generate_broll.py --clip-id <id> --start <s> --end <s>` |
...
- `--concept-index` — use a concept from `scripts/plan_broll.py` output (skips `--prompt`)
...
- **Re-render after adding B-Roll.** The B-Roll overlay is applied during the next clip render. Use `scripts/render_clip.py` after generating B-Roll.
...
- [scripts/generate_broll.py](scripts/generate_broll.py)
- [scripts/plan_broll.py](scripts/plan_broll.py)
