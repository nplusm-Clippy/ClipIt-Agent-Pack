...
Exports are separate from generic jobs. Always poll export jobs with `GET /api/v1/exports/{jobId}` via `scripts/wait_for_export.py`, not `scripts/wait_for_job.py`.

## Quick Reference

| Operation | Preferred path | REST fallback |
|-----------|----------------|---------------|
| Start/wait/download export | `clipit exports start|wait|download ...` | `scripts/start_export.py`; `scripts/wait_for_export.py`; `scripts/download_export.py` |
| Exact enterprise delivery | enterprise exact contract | `scripts/run_enterprise_exact_delivery.py ... --max-credits <cap> --confirm` |
| Deliver/list enterprise output | `clipit deliverables create|list ...` | `scripts/deliver_export.py`; `scripts/list_deliverables.py` |
...
**When to use:** `scripts/start_export.py` returned an `exportJobId` and the export is still pending or processing.
...
- **409 premature download.** The export is not completed yet. Run `scripts/wait_for_export.py` and retry download after completion.
- **404 export not found.** Check the `exportJobId` and that it belongs to the same API key owner.
- **409 editor identity conflict.** Rerun `scripts/start_export.py` so it refreshes delivery-state; never hand-build editor pins.
- **403 enterprise workspace preflight.** Stop and select the exact named workspace profile. Do not fall back to a personal key.
- **409 DELIVERY_EXISTS.** `scripts/deliver_export.py` resolves the existing exact delivery automatically and reports `replayed: true`.
...
- **Failed export.** Inspect the error returned by `scripts/wait_for_export.py`; source media issues may require re-rendering or choosing different export settings.

## Verification

- **Export started:** Response includes `exportJobId`, `status`, and `pollUrl`
- **Export completed:** `scripts/wait_for_export.py` returns `status: "completed"`
- **Download URL available:** `scripts/download_export.py` returns `downloadUrl`
- **Correct poller used:** Export jobs are never polled with `scripts/wait_for_job.py`
...
- **Delivery ready:** `scripts/deliver_export.py` returns one exact-workspace deliverable with `status: "ready"`, or an existing `ready`/`selected` delivery with `replayed: true`
...
- [scripts/create_clip.py](scripts/create_clip.py)
- [scripts/deliver_export.py](scripts/deliver_export.py)
- [scripts/download_export.py](scripts/download_export.py)
...
- [scripts/export_contract.py](scripts/export_contract.py)
...
- [scripts/start_export.py](scripts/start_export.py)
...
- [scripts/wait_for_export.py](scripts/wait_for_export.py)
