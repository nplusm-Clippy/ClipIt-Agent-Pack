...
|-----------|----------------|---------------|
| Generate/update captions | discovered caption tool | `scripts/generate_captions.py`; `scripts/update_captions.py` |
| Set exact enterprise style | enterprise exact contract | `scripts/set_caption_style.py --workspace-id <id> --clip-id <id> --profile <name> ...` |
...
For a workspace clip, never approximate a requested style with one of the four legacy names. Initialize with `--caption-style-json @style.json`, or use `scripts/set_caption_style.py` with the exact version/hash/settings revision returned by initialization. A TikTok Viral request with 2x scale can be expressed as:
...
The directive response replaces the initializer handoff. Do not reuse the initialization identity after `scripts/set_caption_style.py` succeeds. Copy its full normalized `captionStyle` into the style JSON for exact delivery, retain its `captionStyleHash`, and replace all three pinned values with the response's new `editorVersion`, `editorStateHash`, and `clipSettingsRevision`:
...
The mutation must return the normalized `captionStyle`, a 64-character `captionStyleHash`, strict capability metadata, and `renderQueued: false`. Compare every requested field before approval. Then use `scripts/run_enterprise_exact_delivery.py`; direct captioned `scripts/render_clip.py --workspace-id ...` is blocked because the legacy render endpoint can replace the exact style.

## Pitfalls

- **Transcript required.** If you get `TRANSCRIPT_REQUIRED`, transcribe the video first using `scripts/transcribe_video.py`.
- **Changes don't appear in existing renders.** After updating captions, re-render the clip to bake in the new styling.
- **Presets override individual style options.** If you set `--preset bold` and `--font Arial`, the preset's font wins. Apply the preset first, then override specific fields with `scripts/update_captions.py`.
...
- [scripts/generate_captions.py](scripts/generate_captions.py)
...
- [scripts/update_captions.py](scripts/update_captions.py)
