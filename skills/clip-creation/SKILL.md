...
Use with `clipit-operator`. Prefer `clipit videos`, `clipit clips`, `clipit ask`, or discovered MCP tools; use these Python scripts for REST fallback and exact enterprise recipes. Read [references/clipit-operator--editorial-workflow.md](references/clipit-operator--editorial-workflow.md) before selecting or assembling moments.
...
|-----------|----------------|---------------|
| AI clip suggestions | `clipit videos suggest-clips <videoId> --count 5 --confirm --json` | `scripts/suggest_clips.py --video-id <id>` |
| Create/list/get/update/delete | `clipit clips ...` | matching clip scripts |
| Render and wait | `clipit clips render <clipId> --confirm --max-credits <cap> --json`; `clipit jobs wait <jobId>` | `scripts/render_clip.py --clip-id <id> --wait` |
| Initialize/exact enterprise delivery | enterprise Python recipe | `scripts/initialize_editor_snapshot.py`; `scripts/run_enterprise_exact_delivery.py` |
...
**Prerequisites:** The video MUST be transcribed first. If not, use `scripts/transcribe_video.py --video-id <id> --wait` from the video-management skill.
...
4. To create a clip from a suggestion, use the `startTime` and `endTime` values with `scripts/create_clip.py`
...
Never use direct `scripts/render_clip.py --workspace-id ...` for enterprise work; its legacy four-style request can replace exact fields. If a requested field is unsupported, return one product-owned blocked state rather than asking the client to edit and save manually.
...
- **Transcript required for AI suggestions.** `scripts/suggest_clips.py` will fail if the video isn't transcribed. Always transcribe first.
...
- **Changing clip timing invalidates the render.** If you update start/end times with `scripts/update_clip.py`, the previous render is stale — re-render before downloading.
- **Download URLs expire.** Each call to `scripts/download_clip.py` generates a fresh signed URL. Don't cache them.
...
- **Download URL works:** The URL from `scripts/download_clip.py` returns a video file when fetched (Content-Type: video/mp4)
...
- [scripts/download_clip.py](scripts/download_clip.py)
...
- [scripts/update_clip.py](scripts/update_clip.py)
