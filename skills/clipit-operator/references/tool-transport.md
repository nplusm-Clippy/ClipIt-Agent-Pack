...
When `clipit ask` itself is unavailable, `python3 scripts/orchestrate.py "<goal>" --video-id <id>` is its workflow fallback. If it exits awaiting approval, use the exact returned job/approval IDs with `scripts/approve_workflow.py`; do not launch a new orchestration job.
